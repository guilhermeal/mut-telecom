...
<div data-screen-label="Area do cliente">
      <section id="mut-logged-out" style="padding:72px 0; min-height:60vh; display:flex; align-items:center;">
        <div style="max-width:440px; margin:0 auto; padding:0 24px; width:100%;">
          ...
          <form id="mut-login-form" style="background:var(--surface); border:1px solid var(--border); border-radius:20px; padding:28px; display:grid; gap:16px;">
            <div>
              <label style="display:block; font-size:13.5px; font-weight:600; margin-bottom:7px;">CPF</label>
              <input name="cpf" class="mut-input" placeholder="000.000.000-00" style="width:100%; padding:13px 15px; border-radius:11px; border:1.5px solid var(--border); background:var(--background); color:var(--foreground); font-size:15px; font-family:inherit; outline:none;">
            </div>
            <div>
              <label style="display:block; font-size:13.5px; font-weight:600; margin-bottom:7px;">Senha</label>
              <input type="password" name="senha" class="mut-input" placeholder="••••••••" style="width:100%; padding:13px 15px; border-radius:11px; border:1.5px solid var(--border); background:var(--background); color:var(--foreground); font-size:15px; font-family:inherit; outline:none;">
            </div>
            <div id="mut-login-error" class="hidden" style="font-size:13px; color:var(--accent); font-weight:600;"></div>
      ...
      <section id="mut-logged-in" class="hidden sec-pad" style="padding:48px 0 80px;">
        <div style="max-width:1100px; margin:0 auto; padding:0 24px;">
          ...
          <div style="background:var(--background); border:1px solid var(--border); border-radius:18px; overflow:hidden;">
            ...
            <div style="display:grid; grid-template-columns:1.1fr 1fr 1fr 1fr; gap:8px; padding:14px 22px; background:var(--surface); border-bottom:1px solid var(--border); font-size:12.5px; font-weight:700; color:var(--muted); text-transform:uppercase; letter-spacing:.4px;"><div>Vencimento</div><div>Valor</div><div>Status</div><div style="text-align:right;">Ação</div></div>
<?php foreach ($faturas as $b): ?>
            <div style="display:grid; grid-template-columns:1.1fr 1fr 1fr 1fr; gap:8px; padding:16px 22px; border-bottom:1px solid var(--border); align-items:center; font-size:14.5px;"><div style="font-weight:600;"><?= e($b['venc']) ?></div><div style="font-family:'Archivo',sans-serif; font-weight:700;"><?= e($b['valor']) ?></div><div><span style="font-size:12px; font-weight:700; padding:5px 11px; border-radius:999px; color:<?= e($b['cor']) ?>; background:<?= e($b['bg']) ?>;"><?= e($b['status']) ?></span></div><div style="text-align:right;"><a href="segunda-via.php" style="padding:7px 13px; border-radius:9px; border:1px solid var(--border); background:var(--surface); color:var(--primary); font-size:12.5px; font-weight:600; cursor:pointer; text-decoration:none;">2ª via</a></div></div>
<?php endforeach; ?>
          </div>
